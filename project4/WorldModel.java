...
public final class WorldModel {
    ...
    public void parseThanos(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.THANOS_NUM_PROPERTIES) {
            Entity entity = new IronMan(id, pt,  imageStore.getImageList(Entity.THANOS_KEY),
                    Double.parseDouble(properties[Entity.THANOS_ACTION_PERIOD]),
                    Double.parseDouble(properties[Entity.THANOS_ANIMATION_PERIOD]));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.THANOS_KEY, Entity.THANOS_NUM_PROPERTIES));
        }
    }
    public void parseIronMan(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.IRONMAN_NUM_PROPERTIES) {
            Entity entity = new IronMan(id, pt,  imageStore.getImageList(Entity.IRONMAN_KEY),
                    Double.parseDouble(properties[Entity.IRONMAN_ACTION_PERIOD]),
                    Double.parseDouble(properties[Entity.IRONMAN_ANIMATION_PERIOD]));
            this.tryAddEntity(entity);
        }else{
            throw new IllegalArgumentException(String.format("%s requires %d properties when parsing", Entity.IRONMAN_KEY, Entity.IRONMAN_NUM_PROPERTIES));
        }
    }
    ...
    public void parseEntity(String line, ImageStore imageStore) {
        ...
        if (properties.length >= Entity.ENTITY_NUM_PROPERTIES) {
            ...
            switch (key) {
                ...
                case Entity.IRONMAN_KEY -> parseIronMan(properties, pt, id, imageStore);
                case Entity.THANOS_KEY -> parseThanos(properties, pt, id, imageStore);
