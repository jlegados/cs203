            ...
            potentialNeighbors.apply(current.point)
                    ...
                    .forEach(neighbor -> {
                        ...
                        if (!gScore.containsKey(neighbor)){
                            ...
                            int totalCost = tentativeGScore + CalculateHeuristic(neighbor, end);
                            openSet.add(new Node(neighbor, tentativeGScore, totalCost));
                        }
                        else if (tentativeGScore < gScore.get(neighbor)){
                                System.out.println(openSet.remove(new Node(neighbor, gScore.get(neighbor), 0)));
                                cameFrom.put(neighbor, current.point);
                                gScore.put(neighbor, tentativeGScore);
                                int totalCost = tentativeGScore + CalculateHeuristic(neighbor, end);
                                openSet.add(new Node(neighbor, tentativeGScore, totalCost));
